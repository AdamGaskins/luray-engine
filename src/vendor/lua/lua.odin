...
when LUA_SHARED {
	when ODIN_OS == .Windows {
		...
		// liblua5.4.a is linked directly by emcc; "env.o" makes Odin emit
		// the wasm imports under the "env" module so Emscripten's own
		// symbol resolution picks them up, matching vendor:raylib's
		// RAYLIB_WASM_LIB / -define:RAYLIB_WASM_LIB=env.o approach.
	...
	when ODIN_OS == .Windows {
		...
		// See comment in the LUA_SHARED branch above.
		foreign import lib "env.o"
	} else {
		foreign import lib "system:lua5.4"
	}
...
// These are pinned to their 64-bit form (not branched on `size_of(uintptr)`)
// because lua_Number/lua_Integer's size is controlled by the C library's own
// LUA_FLOAT_TYPE/LUA_INT_TYPE (luaconf.h), not by the target's pointer width
// -- the two are unrelated. lib/wasm/liblua5.4.a and the native system Lua
// are both built from stock Lua source without LUA_32BITS, so lua_Number is
// `double` and lua_Integer is `long long` (64-bit) on every platform this
// project links against today, wasm included. Getting this wrong caused a
// real bug: on js_wasm32 (4-byte pointers) these used to resolve to f32/i32,
// while the actual linked library returned f64/i64, corrupting every
// lua.tonumber()/lua.tointeger() call (caught via a wasm-ld function
// signature mismatch warning for lua_tonumberx).
//
// If a build ever links a Lua library actually compiled with -DLUA_32BITS
// (e.g. to shrink Lua's tagged-value size for a memory-constrained wasm
// build), these should be driven by an explicit `#config` flag kept in sync
// with that compile flag -- not inferred from pointer width.
...
foreign lib {
	...
	*/
	...
	// Odin's wasm backend does not support foreign *variables* declared
	// inside a `foreign` block tied to an import library (only foreign
	// procedures are supported that way for wasm targets). `ident` is
	// unused anywhere in this project, so it's only declared on non-wasm
	// targets. This file is vendored from Odin's own Lua bindings — if
	// re-vendoring, re-apply this gate.
	when ODIN_OS != .JS {
